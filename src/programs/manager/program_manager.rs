...
pub mod ffi_program_manager {
    extern "Rust" {
        ...
        #[swift_bridge(associated_to = RHashMapStrings)]
        pub fn new(vector: Vec<RKVPair>) -> RHashMapStrings;
    }
}
...
pub struct RTransactionWrapper(Arc<Mutex<RTransaction>>);
...
impl RProgramManager {
    pub fn r_execute(
        ...
        if let Some(fee_microcredits) = match &fee_record {
            Some(fee_record) => Self::validate_amount(fee_credits, fee_record, true).ok(),
            None => Some((fee_credits * 1_000_000.0) as u64),
        } {
            let mut process_native = ProcessNative::load_web()
            ...
            if let (Some(program_native), Some(process)) = (
                ProgramNative::from_str(program)
                    .map_err(|e| e.to_string())
                    .ok(),
                process,
            ) {
                RProgramManager::resolve_imports(process, &program_native, imports).ok();
                let rng = &mut StdRng::from_entropy();

                // log("Executing program");
                let (a, mut trace) = execute_program!(
                    process,
                ...
                return Some(RTransaction::from(transaction));
            } else {
                return None;
            }
        } else {
...
impl RHashMapStrings {
    fn new(vector: Vec<RKVPair>) -> RHashMapStrings {
